...
C*--------------------------------------------------------------------*C
C* THIS PROGRAM READS D2 HDF FORMAT FILES PRODUCED BY ISCCP           *C
C*--------------------------------------------------------------------*C
C* CONTAINS:                                                          *C
C*    PROGRAM SAMPLE    :  EXAMPLE OF HOW TO USE THESE SUBROUTINES    *C
C*    SUBROUTINE D2OPEN :  POSITION FILE AT REQUESTED SDS             *C
...
C*--------------------------------------------------------------------*C
C*  Modifications                                                     *C
C*    April 2002 - The format of the ISCCP D2 hdf files was modified  *C
...
C*--------------------------------------------------------------------*C
C* LOOP OVER UTC FOR FILE POSITIONING                                 *C
C*--------------------------------------------------------------------*C
      DO 600 IUTC = 9,9
      CALL D2OPEN(HDFFILE,IUTC,IRC)
C*--------------------------------------------------------------------*C
...
C* FOR THIS SAMPLE PROGRAM, JUST SELECT A FEW BOXES                     
         IF ( LAT .GE. 1 .AND. LAT .LE. 52 ) THEN                       
...
C**********************************************************************C
C* NAME:         D2OPEN                                               *C
C* DESCRIPTION:  OPEN HDF FILE AND FIND REQUESTED SDS                 *C
C**********************************************************************C
C**********************************************************************C
C**********************************************************************C
      SUBROUTINE D2OPEN(HDFFILE,IUTC,IRC)
      CHARACTER*(*) HDFFILE
      INTEGER       IREF, ITAG/720/
      INTEGER       ID_HDF
      INTEGER       HOPEN,DAGFIDL,DAGFID,DSRREF,DAGLLEN,DAGLAB,HCLOSE
      INTEGER       DALLIST
     ...
     $                        ST_POS)
      ...
      PRINT *,'FILE POSITIONED AT DATA LABEL: ',LABEL(1:LENGTH)
...
C**********************************************************************C
C**********************************************************************C
      SUBROUTINE D2READ(HDFFILE,IRC)
      PARAMETER     ( MAXVAR = 130 )                                    
      PARAMETER     ( MAXLAT =  72 )                                    
      PARAMETER     ( MAXLON = 144 )                                    
      PARAMETER     ( IUNDEF = 255 )                                    
      PARAMETER     ( RUNDEF = -1000.0 )                                
      COMMON /D2BUFS/ CHRBUF(MAXVAR)                             
      CHARACTER*1     CHRBUF                                            
      COMMON /D2DATA/ LAT,NLON,IVAR(MAXVAR,MAXLON),RVAR(MAXVAR,MAXLON)  
      COMMON /D2GRID/ NCELLS(MAXLAT),ICELLS(MAXLAT)                     
      CHARACTER*(*) HDFFILE
...
C**********************************************************************C
C* NAME:  RDANC                                                       *C
C* DESCRIPTION:  READ D2 ANCILARY DATA FILE (FILE DESCRIPTION 4)      *C
C**********************************************************************C
C**********************************************************************C
C**********************************************************************C
...
C**********************************************************************C
  ...
  140 FORMAT(/1X,'PRINTR:  PHYSICAL VALUES FOR ALL VARIABLES')          
      PRINT 145,(K,K=1,10)                                              
  145 FORMAT(1X,18X,10I8)                                               
C     DO 150 I=1,MAXVAR,10                                              
C        IEND = I + 9                                                   
C        IF ( IEND .GT. MAXVAR ) IEND = MAXVAR                          
C        PRINT 155,I,IEND,(RVAR(K,LON),K=I,IEND)                        
C 150 CONTINUE                                                          
C 155 FORMAT(1X,'VARIABLE (',I3.3,'-',I3.3,')',10F8.2)                  
      RETURN                                                            
      END                                                               
C**********************************************************************C
C**********************************************************************C
